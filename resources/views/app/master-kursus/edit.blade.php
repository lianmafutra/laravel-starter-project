...
@endpush
@section('header')
    <x-header title="Edit Master Kursus" back-button="true"></x-header>
@endsection
@section('content')
    ...
    <script>
        $(function() {
            ...
            $('#form_sample').submit(function(e) {
                ...
                $.ajax({
                    type: 'POST',
                    url: route('master-kursus.update', @json($masterKursus->uuid)),
                    data: formData,
                    contentType: false,
                    processData: false,
                    dataType: 'json',
            ...
            file_cover.setOptions({
                ...
                files: @json($masterKursus->field('file_cover')->getFilepond()),
                beforeRemoveFile: (file) => {
                    return confirm("Are You Sure Delete This File ?");
                },
            })

            $('#judul').val(@json($masterKursus->judul))
            $('#deskripsi').val(@json($masterKursus->deskripsi))
            if (@json($masterKursus->active) == "true") {
