    ...
    <script>
        $(function() {
            ...
            $('#form_sample').submit(function(e) {
                ...
                $.ajax({
                    type: 'POST',
                    url: route('master-paket-soal.update', @json($masterPaketSoal->uuid)),
                    data: formData,
                    contentType: false,
                    processData: false,
                    dataType: 'json',
            ...
            // set data
            $('#judul').val(@json($masterPaketSoal->judul))
            $('#deskripsi').val(@json($masterPaketSoal->deskripsi))
            $('#durasi').val(@json($masterPaketSoal->durasi))

            if (@json($masterPaketSoal->active) == "true") {
               $('#active_1').prop('checked', true);
            } else {
               $('#active_2').prop('checked', true);
            }
            $("#jenis_paket").val(@json($masterPaketSoal->jenis_paket)).change()
            $("#master_kursus_id").val(@json($masterPaketSoal->master_kursus->id)).change()
